...
//c3pos2: input
void singleNodeLaplacians(ap_int<12> a1[nNodes], ap_int<12> a2[nNodes], ap_int<20> laplacians[nNodes][nNodes], ap_int<20> laplaciansT[nNodes][nNodes], int16_t c3pos2){


	singleNodeLaplacians_label0:for(int i=0; i<nNodes; i++){
#pragma HLS UNROLL

			int tag_prev = i<<4;
		singleNodeLaplacians_label1:for(int j=0; j<nNodes; j++){
#pragma HLS UNROLL
				int temp = std::abs(a1[i] + a2[j] - c3pos2);
				int temp1 = temp>>8;
				int tag = tag_prev + j;
				int temp2 = temp1 + tag;
...
void LaplacianCalc(const ap_int<12> c1coord[5][nNodes], const ap_int<12> c2coord[5][nNodes],const ap_int<12> c3coord[5][nNodes], int c1, int c2, int c3, int c4, int* ans){

	ap_int<20> laplaciansIO[3][nNodes][nNodes][nNodes];
	ap_int<20> laplaciansIO_T[3][nNodes][nNodes][nNodes];
	...
	allLaplacians_label2:for(int i=0; i<nNodes; i++){
#pragma HLS UNROLL
