...
void generateCoordinates(float layerRadii[totalLayers], short numberOfCollisionsPerTime, short numberOfSections, short outcome, float curveRadius,
	...
	double pixelWidthForSmearingPhi = 0.0000; // in cm
	double pixelWidthForSmearingZ = 0.0000; // in cm
	...
	double zSliceWidth = 40.0; // width of z slice in cm for layer[0]; TODO: could make this array of different numbers for z width per layer
	...
	const float lambdaScale = 1000.0;
	float ptKickAngular = (0.0136*radiationLengthPerLayer)/momentum;

	std::default_random_engine generator;
    // Create a normal_distribution object with the specified mean and standard deviation
	...
	for (short layer = 0; layer < totalLayers; layer++) { 
		...
		if (layer == 0) {
			...
			randomZ1 = randomFloat(-maxNumPixelsLayerZ/2.0, maxNumPixelsLayerZ/2.0);
			randomLambda1 = randomFloat(-lambdaRange*lambdaScale/2.0, lambdaRange*lambdaScale/2.0);
			//cout << "RandomLambda before scaling: " << randomLambda1 << "\n";
			randomLambda1 = randomLambda1/lambdaScale;
			lambda = randomLambda1;

   			curvedLine2 = random((short)(-maxNumPixelsLayerPhi/2), (short)(maxNumPixelsLayerPhi/2)); 
		}
		double gaussianRandomPhi = distribution(generator); // implement multiple scattering in phi
		...
		thetaSmearMS += gaussianRandomPhi;
		thetaSmearMS /= layerRadii[layer];

		double gaussianRandomZ = distribution(generator); // implement multiple scattering in z
		...
		//curveThetaSmear += thetaSmearMS; TODO: fix this
		//curveZ += zSmearMS;
		...
		randomHitsVectorPhi = getRandomHits((short)(-maxNumPixelsLayerPhi/2), (short)(maxNumPixelsLayerPhi/2));
		...
		randomHitsVectorZ = getRandomHits((short)(-maxNumPixelsLayerZ/2), (short)(maxNumPixelsLayerZ/2));
