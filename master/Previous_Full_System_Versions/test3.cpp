...
void generateCoordinates(int layerRadii[totalLayers], double theta[numberOfNodes], int fragmentsPerCollision, int numberOfCollisionsPerTime, int timeBetweenCollisions, double pixelWidth, int numberOfSections, int outcome, int curveRadius) {
	...
	for (int layer = 0; layer < totalLayers; layer++) {
		...
		if (outcome == 0) {
            ...
            coordinates[layer][5] = curvedLine1 + (int)(10000/25)*((double)curveRadius - abs(((double)curveRadius)*cos(curveTheta))); 
            curvedLine1 = coordinates[layer][5];
            CurvedFile <<  curvedLine1 << "\n";
		} else if (outcome == 1) {
            coordinates[layer][5] = curvedLine1 + (int)(10000/25)*((double)curveRadius - abs(((double)curveRadius)*cos(curveTheta)));  
            curvedLine1 = coordinates[layer][5];
			coordinates[layer][12] = curvedLine2 + (int)(10000/25)*((double)curveRadius - abs(((double)curveRadius)*cos(curveTheta))); 
...
void MSS(T work_array[], int const &power) {
    ...
    if (power == 1) {
        compareAndExchange(work_array, 0, 1, true);
    } 
    if (power >= 2) {
...
void fullSystem(const int nodeCoordinates[totalLayers][numberOfNodes], bool MM[middleLayers][numberOfNodes][adjacentLayers][numberOfNodes], int laplacianWeights[2]) {
    ...
    int tripletMatrix[middleLayers][numberOfNodes][16][16];
    ...
    int tripletMatrixTranspose[middleLayers][numberOfNodes][16][16];
		...
		for (int middleLayer=0; middleLayer<middleLayers; middleLayer++) {
			...
			for (int nodeIndex=0; nodeIndex<numberOfNodes; nodeIndex++) {
                ...
                if (power == 4) {
                    ...
                    memcpy(tripletMatrixCopy, tripletMatrix, middleLayers*numberOfNodes*(int)pow(2, power)*(int)pow(2, power)*sizeof(int));
                    memcpy(tripletMatrixTransposeCopy, tripletMatrixTranspose, middleLayers*numberOfNodes*(int)pow(2, power)*(int)pow(2, power)*sizeof(int));
                }
